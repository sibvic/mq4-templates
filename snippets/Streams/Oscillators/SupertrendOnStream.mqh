...
public:
   ...
   virtual bool GetValue(const int period, const double factor, double &supertrend, double &direction)
   {
      EnsureSize();
      if (!ComputeTo(period, factor))
      {
         return false;
      }
      supertrend = _supertrend[period];
      direction = _direction[period];
      return true;
   }
...
private:
   ...
   bool ComputeBar(const int p, const double factor)
   {
      ...
      double atrVal;
      if (!_atr.GetValue(p, atrVal))
      {
         return false;
      }

      double src = (h + l) / 2.0;
      double upperBand = src + factor * atrVal;
      double lowerBand = src - factor * atrVal;
      int totalBars = _source.Size();
